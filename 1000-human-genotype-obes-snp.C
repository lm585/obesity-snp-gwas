  ...
  while(!input.eof())
  {
   ...
   else //already encounted the header line
   {
     ...
     (toupper(snpMap[lineFields[2]]) == toupper(lineFields[3][0]) || 
      toupper(snpMap[lineFields[2]]) == toupper(lineFields[4][0]) ) ) 
      ...
      //re ID risk allele same as REF or ALT allele of this vcf line
     {
      ...
      {
       twoInt zz;
       zz.gwasAllelFreq = 0; zz.altAllelFreq = 0;
       allelFreq[*it] = zz; 
      }
      ...
      for(i = 9; i < lineFields.size(); i++) //genotype field starts with 10th field
      {
       ...
       if(idPopu.count(vcfHeader[i]) > 0)
       {
        ...
        if(toupper(snpMap[lineFields[2]]) == toupper(lineFields[3][0])) //re ID risk allele same as REFallele
        {
         if(al12vect[0] == "0")
        ...
        else if(toupper(snpMap[lineFields[2]]) == toupper(lineFields[4][0])) //re ID risk allele same as ALT allele
        {
         if(al12vect[0] == "0") //REF allele, but diff re ID allele
           allelFreq[popF].altAllelFreq++;
         if(al12vect[0] == "1") //ALT allele, same as rs ID allele
           allelFreq[popF].gwasAllelFreq++;
         if(al12vect[1] == "0")
           allelFreq[popF].altAllelFreq++;
         if(al12vect[1] == "1")
      ...
      if(toupper(snpMap[lineFields[2]]) == toupper(lineFields[3][0])) //re ID risk allele same as REF
        output << (char) toupper(lineFields[4][0]) << t;
      else
        output << (char) toupper(lineFields[3][0]) << t;
      ...
      output << 1.0 * ar / (ar + aa) ;
      output << endl;
     } //end of ... snpMap.count(lineFields[2]) > 0 ...
   }
   getline(input, line);
  }
  input.close();
